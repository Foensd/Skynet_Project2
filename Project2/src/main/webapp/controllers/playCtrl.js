...
myApp.controller('PlayController', [
		...
		function($scope, $http) {
			...
			// var REQUEST_SERVICE_URI =
			// 'http://13.59.197.145:8085/Project2/#/play';
			...
			$scope.register = function() {
				...
				/*
				 * var path = "/Project2/index.html#/lobby"; location.href =
				 * path;
				 */

				/*
				 * .then(function(playerObject) { console.log("SUCCESSFULLY
				 * ENTERED A USER"); //$scope.user = data; });
				 */
			}
		} ])
/*
 * .factory("PlayerService", ['$http', function($http){ //Creating factory named
 * "PlayerService debugger;
 * 
 * 
 * playerObject.AddPlayerToDB = function(user) { $http.post(REQUEST_SERVICE_URI,
 * user).then(function (response) { alert(response.status); }) } //rest request
 * return playerObject;
 * 
 * }])
 */
